...
async def generate_random_string_for_orderid():
    """ Генерация случайной строки для Order.order_id """
    return "".join(
...
async def create_order(order_products: orders_schema.OrderItems, session: AsyncSession):
    ...
    for order_product in order_products["products"]:
        product = await products_utils.get_product_by_id(
            order_product["product_id"], session
        )

        # Проверка на отрицательное количество заказанного товара
        if order_product["quantity"] < 0:
            await session.rollback()
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Product quantity has to be positive')
        
        # Проверка наличия нужного количества товара в бд
        if product.quantity < order_product["quantity"] and product.quantity > 0:
            await session.rollback()
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f'Product with ID `{order_product["product_id"]}` not enough',
            )
        ...
        # Убавление количества товаров из бд
...
async def edit_order_status(order_id: str, session: AsyncSession):
    order = (
